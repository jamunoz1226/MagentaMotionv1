...
const GlassCard: React.FC<GlassCardProps> = ({
  ...
  const baseClasses = cn('relative overflow-hidden rounded-xl', blurClasses[blur], opacityClasses[opacity], shadowClasses[shadow], border && 'border border-white/20', hover && 'hover:bg-white/15 transition-all duration-300 hover:shadow-xl hover:shadow-black/30', onClick && 'cursor-pointer', className);
  const cardContent = <div className={baseClasses} onClick={onClick} data-magicpath-id="0" data-magicpath-path="GlassCard.tsx">
      {/* Gradient overlay for extra depth */}
      <div className="absolute inset-0 bg-gradient-to-br from-white/10 via-transparent to-black/10 pointer-events-none" data-magicpath-id="1" data-magicpath-path="GlassCard.tsx" />
